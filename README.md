...
This project explores how **hierarchical Bayesian models** can be used for **behavioral anomaly detection** in security event counts. Instead of classifying attacks, it models what is **normal for each entity** and flags **statistically improbable count behavior** under rare-event regimes — designed to **reduce alert fatigue** and **prioritize analyst attention** when attacks are rare (<5%).

**Key Results** (from reproducible benchmarks in `outputs/`):
- **12× fewer alerts** than Random Forest at equal recall (1% attack rate, CSE-CIC-IDS2018)
- **PR-AUC 0.562 vs 0.283** for Z-Score baseline (2% attack rate, synthetic)
- **<8% performance drift** across two held-out time periods (no weekly retraining needed)
- **Precision@100 = 0.92** with alert budget calibration (triage layer)
...
| Path | Notebook | What You'll Learn |
|------|-----------|-------------------|
...
| **🎯 SOC Operations** | [`04_alert_prioritization.ipynb`](notebooks/04_alert_prioritization.ipynb) | Risk scoring, alert budgets, entity context, Precision@k — all with synthetic data |
| **🧑‍💼 Interview Prep** | [`05_interview_prep_fortra.ipynb`](notebooks/05_interview_prep_fortra.ipynb) | 6 hard technical questions answered with runnable code (MCMC diagnostics, data leakage, production architecture) |
| **🇪🇸 Deep Dive (ES)** | [`06_explicacion_profunda.ipynb`](notebooks/06_explicacion_profunda.ipynb) | Interactive Spanish walkthrough: problem → model → scoring → triage, all sections executable |

### 📖 Deep Dives

| Document | Purpose |
|----------|---------|
| [`LEAME_PROFUNDO.md`](LEAME_PROFUNDO.md) | **Explicación completa en español** — 10 secciones: el problema, modelo matemático, pipeline, scoring, evaluación, triage, limitaciones honestas |
| [`BLOG_POST_LINKEDIN.md`](BLOG_POST_LINKEDIN.md) | **Technical blog post** — results-first narrative for LinkedIn/Medium, all numbers backed by benchmark runs |
| [`docs/assets/unsw_nb15_dataset_description.md`](docs/assets/unsw_nb15_dataset_description.md) | What are network flows? Dataset documentation |
...
Pedagogical visualizations generated by `scripts/explain_pipeline.py` — saved to `outputs/explanation/`:
...
| Figure | Content |
|--------|---------|
| `01_el_problema.png` | Entity heterogeneity, attack rarity, overdispersion |
| `02_falla_metodos_clasicos.png` | Why Z-Score / flat thresholds fail |
| `03_modelo_jerarquico.png` | Plate diagram + partial pooling visualization |
| `04_pipeline_datos.png` | Raw events → modeling table, step by step |
| `05_anomaly_scoring.png` | −log P(y\|posterior), class separation |
| `06_evaluacion.png` | ROC vs PR-AUC, Precision@k, alert budget curve |
| `07_triage.png` | Risk score, threshold calibration, SOC workflow |
| `08_limitaciones.png` | Honest detectability map by attack type |

For a narrative companion, see [`LEAME_PROFUNDO.md`](LEAME_PROFUNDO.md) (Spanish, 10 sections) or [`BLOG_POST_LINKEDIN.md`](BLOG_POST_LINKEDIN.md) (English blog post).
...
| **05. Interview Prep** | MCMC diagnostics, NB vs Poisson, data leakage, cold-start, production architecture | 6 senior-level Q&A with runnable code |
| **06. Explicación Profunda (ES)** | Pipeline completo en español: problema → modelo → scoring → triage | 7 secciones interactivas, todas ejecutables |
...
```bash
# Tests
pytest tests/ -v -m "not slow"           # 65 tests, ~30s
pytest tests/ -v                          # includes slow MCMC tests

# Benchmarks
make demo-fast                            # Quick end-to-end demo
make benchmark                            # Full model comparison (all baselines)
make robustness                           # Drift, cold-start, attack rate sweep

# Visualizations
python scripts/explain_pipeline.py        # 8 pedagogical figures → outputs/explanation/
python scripts/alert_prioritization.py   # Triage dashboard → outputs/triage/

# Multi-regime real data (requires CSE-CIC-IDS2018)
python scripts/multi_regime_comparison.py

# Notebooks
python scripts/build_notebooks.py        # Regenerate notebooks 04, 05, 06
```

All benchmark results in `outputs/`; see `outputs/benchmark/summary_metrics.csv` for the main comparison table.
...
│   │   ├── data_generator.py      # GeneratorConfig + synthetic data
│   │   ├── features/              # FeatureConfig + feature engineering
│   │   ├── model.py               # ModelConfig + build/fit/diagnose
│   │   ├── scoring/               # Posterior anomaly scoring
│   │   ├── evaluation/            # PR-AUC, Recall@k, operational metrics
│   │   ├── steps.py               # Thin shim: Settings → module configs
│   │   ├── pipeline.py            # Pipeline orchestrator + PipelineState
│   │   ├── config.py              # Settings dataclass (all params)
│   │   ├── io.py                  # RunMetadata + I/O helpers
│   │   ├── cli.py                 # Typer CLI (`bsad` command)
│   │   ├── baselines.py           # NB_MLE, NB_EmpBayes, GLMM_NB, IF, LOF
...
│   ├── 04_alert_prioritization.ipynb   # Triage con datos sintéticos
│   ├── 05_interview_prep_fortra.ipynb  # 6 Q&A técnicas con código
│   └── 06_explicacion_profunda.ipynb   # Deep-dive interactivo en español
├── scripts/
│   ├── alert_prioritization.py     # Triage dashboard generation
│   ├── multi_regime_comparison.py  # Multi-attack-rate validation
│   ├── explain_pipeline.py         # 8 pedagogical matplotlib figures
│   └── build_notebooks.py          # Programmatic notebook generation
├── outputs/
│   ├── datasets/                   # Results organized by dataset
│   │   ├── synthetic/
│   │   ├── unsw-nb15/
│   │   └── cse-cic-ids2018/
│   ├── benchmark/                  # Full model comparison results
│   ├── robustness/                 # Drift, cold-start, stability
│   ├── triage/                     # Alert prioritization outputs
│   └── explanation/                # 8 pedagogical figures (ES)
├── CLAUDE.md                       # Architecture guide for AI assistants
├── LEAME_PROFUNDO.md               # Explicación completa en español
├── BLOG_POST.md                    # Technical paper-style writeup
├── BLOG_POST_LINKEDIN.md           # LinkedIn/Medium adapted post
└── README.md
```
...
2. **12× fewer alerts** than supervised methods at equal recall (1% attack rate)

3. **Partial pooling > no pooling**: +17% PR-AUC improvement vs NB-MLE with the same likelihood — all from the hierarchical prior

4. **Not for classification** — if attack rate >10%, use Random Forest or XGBoost

5. **Statistical regime matters** more than the dataset itself

6. **Uncertainty quantification** enables confident triage: Precision@100 = 0.92 with alert budget calibration
